...
def main():
    ...
    parser.add_argument("--CW_kappa", type=float, default=0,
    ...
    data = tf.placeholder(tf.float32, shape=(
        None,) + config.image_shape + (config.nr_channel, ), name='data')
    # placeholder for targeted label
    label = tf.placeholder(tf.int32, shape=(None, ), name='label')
    gt = tf.placeholder(tf.int32, shape=(None, ), name='gt')

    pre_noise = tf.Variable(tf.zeros(
        (config.minibatch_size, config.image_shape[0], config.image_shape[1], config.nr_channel), dtype=tf.float32))
    model = Model()
    ...
    train_writer = tf.summary.FileWriter(os.path.join(
        config.log_dir, 'tf_log', 'train'), tf.get_default_graph())
        ...
        for idx in range(train_set.minibatches):
            ...
            for epoch in range(1, args.nr_epoch + 1):
                ...
                if global_cnt % config.show_interval == 0:
                    ...
                    print(
                        "e:{}/{}, {}".format(idx,
                                             train_set.minibatches, epoch),
                        'loss: {:.3f}'.format(loss_batch),
                        'acc: {:3f}'.format(accuracy),
                    )

                successful = acc_gt.eval(feed_dict={placeholders['data']: adv_examples, placeholders['gt']: labels}) > 0.5 if args.targeted_attack else acc_gt.eval(
                ...
                if successful:
                    ...
                    if distortion < min_distortion:
                        ...
                        min_linf = min(min_linf, np.max(
                            (adv_examples - images) / 255))
